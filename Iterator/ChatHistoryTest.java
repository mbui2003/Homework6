...
public class ChatHistoryTest {
    ...
    public void testIterator() {
        Message message1 = new Message("Janet", List.of("Bob"), "Hello Bob!");
        Message message2 = new Message("Bob", List.of("Janet"), "Hello Janet!");
        chatHistory.addMessage(message1);
        chatHistory.addMessage(message2);
        ChatServer chatServer = new ChatServer();

        Iterator<Message> iterator = chatHistory.iterator(new User("Janet", chatServer));
        List<Message> messagesFound = new ArrayList<>();
        while (iterator.hasNext()) {
            messagesFound.add(iterator.next());
        }

        assertEquals(2, messagesFound.size());
        assertTrue(messagesFound.contains(message1));
        assertTrue(messagesFound.contains(message2));
    }
}